...
Room combat currently covers players fighting on foot in the same world, with base class loadouts. Rifle damage/speed/radius and headshot values match the existing base weapons; the multiplayer bow uses a fixed 600-unit range rather than trusting a client's viewport width. Movement uses bounded room positions with a distance budget; map obstacle simulation, NPC combat, vehicles, equipment upgrades, and progression remain single-player systems. A defeated player stays dead until leaving/rejoining a newly spawned room. Leaving removes owned projectiles/deployables, and a replacement player causes a fresh spawn and combat state.

`modules/combat-controls.js` routes existing input actions to the optional `combatSession` transport only while a room is spawned. `multiplayer-game.js` applies server results even while menus are open. Client combat simulation, regeneration, pickups, and automatic respawn are bypassed in that session. Existing teleporter interactions stay active. `portal-travel.cjs` validates the source portal and destination spawn; world changes are sent reliably without the ordinary movement throttle. Travel clears owned projectiles/effects/deployables while preserving server health, ammo and cooldowns. Server projectiles and effects are rendered separately and never enter local collision arrays. Single-player combat functions, weapon balance and projectile definitions are unchanged. The adapter restores pre-session hero combat state on leaving; multiplayer ammo/cooldowns are not written into single-player saves.
...
Teleporter regression coverage: `tests/portals.cjs` checks destination parity with actual world functions, rejects arbitrary jumps, and verifies combat-state preservation. `tests/portals-browser.cjs` walks via keyboard through village/main and village/training round trips with normal collision handling, checking that the second browser receives each world transition. Restart the Node server after server-side changes and reload both clients.

Server-owned enemies: `server-enemies.cjs` owns room-scoped IDs, main-world encounters and spawn tiles, two skeleton waves, pursuit/retreat, nearest living same-world targets, and attack deadlines. The combat engine resolves player/enemy projectiles, damage and death; snapshots include enemies and attack state. Multiplayer rendering reads only this shared list, while local enemy spawners are bypassed. Single-player enemy code and balance are unchanged.
